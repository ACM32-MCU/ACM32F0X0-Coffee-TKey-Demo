...
uint32_t HAL_TIMER_Base_Init(TIM_HandleTypeDef * htim)
{
	...
	if (htim->Init.ARRPreLoadEn)  //重加载 使能
	{
	...
	htim->Instance->ARR = htim->Init.Period;     //装入加载值
	htim->Instance->PSC = htim->Init.Prescaler;  //时钟分频值 
	if (IS_TIM_REPETITION_COUNTER_INSTANCE(htim->Instance))
	{
		htim->Instance->RCR = htim->Init.RepetitionCounter;   
	}
	htim->Instance->EGR = BIT0;     // no UIF generated because URS=1  目的是清零计数器
