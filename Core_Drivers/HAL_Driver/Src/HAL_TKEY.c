...
//    if(!IS_TKEY_ALL_INSTANCE(TKEY)) return HAL_ERROR;
...
HAL_StatusTypeDef HAL_TKEY_Suspend(TKEY_HandleTypeDef* htkey)
{
    ...
    for(ucI = 0; htkey->ChannelData[ucI].ChannelId != 0xFFFF; ucI++)
    {
        ...
        *(gu32RegTemp + htkey->ChannelData[ucI].ChannelId) = htkey->ChannelData[ucI].Tkey_Data->RawData;
...
void HAL_TKEY_Calculate_Calibrate_Th(const TKEY_ChannelDataDef *ChannelData)
{
    ...
    if(data < 30)
    {
        ratio = TKEY_CALIBRATE_RATIO40;
    }
    else if((data >= 30) && (data < 50))
    {
        ratio = TKEY_CALIBRATE_RATIO30;
    }
    else
    {
        ratio = TKEY_CALIBRATE_RATIO20;
    }
...
HAL_StatusTypeDef HAL_TKEY_Init(TKEY_HandleTypeDef* htkey)
{
    ...
    u32RegTemp =  ((TKEY_CR_CHARGESEL_LDO << 11)& TKEY_CR_CHARGESEL) | \
                  (TKEY_CR_VKEYSEL(TKEY_CR_VKEYSEL_1V2) & TKEY_CR_VKEYSEL_MASK) | \
                  (TKEY_CR_VREFSEL(TKEY_CR_VREFSEL_1V0) & TKEY_CR_VREFSEL_MASK) | \
    ...
    for(ucI = 0; htkey->ChannelData[ucI].ChannelId != 0xFFFF; ucI++)
    {
        ...
        switch(ChannelData->Tkey_Data->StateId)
        {
            ...
            case TKEY_STATEID_DETECT : 
                ...
                ChannelData->Tkey_Data->StateId = TKEY_STATEID_DETECTING;//必须改变状态，不然偶尔会出现多键
        ...
        switch(ChannelData->Tkey_Data->StateId)
        {
            case TKEY_STATEID_RELEASE : 
                ...
                else
                {
                    ChannelData->Tkey_Data->StateId = TKEY_STATEID_RELEASE;
                    ChannelData->Tkey_Data->DebIn = TKEY_DEBIN;
                    (ChannelData->Tkey_CalData->ScanTimer)++;
            ...
            case TKEY_STATEID_DETECTING : 
                ...
                {
                    if((gu32_SystemCount - ChannelData->Tkey_CalData->DetectingTimer) > TKEY_DETECTING_TIME)//按键长时间未释放就会触发快速自校准
                    {
